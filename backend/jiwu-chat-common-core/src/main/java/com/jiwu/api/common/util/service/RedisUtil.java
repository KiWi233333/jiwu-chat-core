...
public class RedisUtil<K extends Object, V extends Object> {
    @Autowired
    ...
    public void set(String key, Object value, long val, TimeUnit timeUnit) {
    ...
    public Long lRightPushAll(String key, Collection<V> value, TimeUnit timeUnit) {
        try {
            return redisTemplate.opsForList().rightPushAll(key, value, timeUnit);
        } catch (Exception e) {
            return 0L;
        }
    }
    ...
    private final String[] IGNORE_KEYS = new String[]{UserConstant.USER_REFRESH_TOKEN_KEY,// token
            "res:user:", // 资源文件
            "oss:upload:", // 资源文件
    };
     ...
     * @return
     */
    public Long flushDbNoAuth() {
        Set<String> keys = redisTemplate.keys("*");
        // 过滤 user
        keys.removeIf(key -> Arrays.stream(IGNORE_KEYS).anyMatch(key::contains));
        return redisTemplate.delete(keys);
    }
